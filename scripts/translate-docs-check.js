  ...
  async translateFile(contentPath) {
    try {
      ...
      if (!this.needsUpdate(contentPath, outputPath)) {
        if (this.verbose) {
          console.log(`Skipped (up to date): ${targetRelativePath}`);
        }
        this.skippedFiles++;
        return false;
      }
      */
      ...
      // 1. 彻底移除所有的 @@switch 块及其后面的 JS 内容
      // NestJS 文档规范：@@switch 后是 JS 分支。
      // 我们需要移除所有 @@switch 实例及其紧跟的内容，直到遇到下一个 ``` 或双换行（正文开始）
      content = content.replace(/@@switch[\s\S]*?(?=```|\n\n|$)/g, '');

      // 2. 处理标准的 @@filename 模式转换为兼容格式
      // 我们需要捕获 @@filename 标记以及其后紧跟的（可能带有语言标识的）代码块起始符
      content = content.replace(/\n?@@filename\s*\(([^)]*)\)\s*\n?```\w*/g, (match, filename) => {
        return `\n\n\`\`\`typescript\n// @filename(${filename})\n`;
      });
      ...
      console.log(`✅ Processed: ${relativePath} -> ${targetRelativePath}`);
