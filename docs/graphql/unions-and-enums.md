...
To define a GraphQL union type, we must define classes that this union will be composed of. Following the [example](https://www.apollographql.com/docs/apollo-server/schema/unions-interfaces/#联合类型) from the Apollo documentation, we'll create two classes. First, `Book`:
