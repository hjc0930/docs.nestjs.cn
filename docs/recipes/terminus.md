...
Our health check will now send a _GET_-request to the `./` address. If
...
In case your app uses [multiple databases](techniques/database#多个数据库), you need to inject each
...
> info **Info** If you want to learn more about custom loggers in NestJS, [read more here](/techniques/logger#注入自定义日志记录器).
